<?php

declare(strict_types=1);

namespace ReceiptValidator\Tests\AppleAppStore;

use PHPUnit\Framework\Attributes\CoversClass;
use PHPUnit\Framework\Attributes\DataProvider;
use PHPUnit\Framework\TestCase;
use ReceiptValidator\AppleAppStore\ReceiptUtility;

#[CoversClass(ReceiptUtility::class)]
final class ReceiptUtilityTest extends TestCase
{
    #[DataProvider('transactionReceiptProvider')]
    public function testExtractTransactionIdFromTransactionReceipt(string $base64Receipt, ?string $expected): void
    {
        $tx = ReceiptUtility::extractTransactionIdFromTransactionReceipt($base64Receipt);
        self::assertSame($expected, $tx);
    }

    public static function transactionReceiptProvider(): array
    {
        return [
            'valid legacy receipt' => [
                // contains "purchase-info" -> base64(JSON-like) with "transaction-id" = "33993399"
                'ewoicHVyY2hhc2UtaW5mbyIgPSAiZXdvaWRISmhibk5oWTNScGIyNHRhV1FpSUQwZ0lqTXpPVGt6TXprNUlqc0tmUW89IjsKfQo=',
                '33993399',
            ],
            'invalid base64' => [
                '***not-base64***',
                null,
            ],
            'missing purchase-info' => [
                base64_encode('{"nope": "x"}'),
                null,
            ],
        ];
    }

    #[DataProvider('appReceiptProvider')]
    public function testExtractTransactionIdFromAppReceipt(string $appReceipt, ?string $expected, bool $throws): void
    {
        if ($throws) {
            $this->expectException(\ValueError::class);
        }

        $tx = ReceiptUtility::extractTransactionIdFromAppReceipt($appReceipt);

        if (!$throws) {
            self::assertSame($expected, $tx);
        }
    }

    public static function appReceiptProvider(): array
    {
        return [
            'valid PKCS7 with one in-app tx id' => [
                // from your fixture; should yield '0'
                '',
                '0',
                false,
            ],
            'valid PKCS7 with no in-apps' => [
                // your “no transactions” sample — expect null
                '',
                null,
                false,
            ],
            'invalid base64 (throws)' => [
                '***not-base64***',
                null,
                true,
            ],
        ];
    }
}
